/**
 * Совместимость с разными MCP клиентами
 * Адаптирует схемы инструментов под возможности различных клиентов
 */
...
// Пресеты для известных клиентов
...
/**
 * Парсит встроенные JSON строки в объекты
 */
  ...
  for (const [key, value] of Object.entries(newArgs)) {
    if (typeof value === 'string') {
      try {
        ...
        // Парсим только если результат - простой объект (не массив, null или примитив)
...
/**
 * Обрезает имена инструментов до указанной длины, обеспечивая уникальность
 */
...
/**
 * Удаляет union типы верхнего уровня, разделяя инструмент на несколько вариантов
 */
...
/**
 * Инлайнит все $ref ссылки в схеме, устраняя $defs
 */
...
function inlineRefsRecursive(
  ...
  if ('$ref' in result && typeof result.$ref === 'string') {
    if (result.$ref.startsWith('#/$defs/')) {
      ...
      // Если мы уже видели эту ссылку в пути, это циклическая ссылка
...
/**
 * Удаляет anyOf поля из схемы, используя только первый вариант
 */
...
/**
 * Удаляет format поля из схемы и добавляет их в описание
 */
...
/**
 * Применяет все трансформации совместимости к endpoints на основе возможностей клиента
 */
export function applyCompatibilityTransformations(
  ...
  // Обрабатываем union верхнего уровня первыми, так как это меняет имена инструментов
